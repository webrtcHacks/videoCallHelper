...
export async function formatForDisplay(events) {
    ...
    events.forEach(event => {
        ...
        if (message === 'local_audio_level') {
            const audioLevelData = {timeString: datetime, ts: timeStamp, level: data.audioLevel};
            localLevels.push(audioLevelData)
            debug(datetime, event.message, event.data.audioLevel)
        } else if (message === 'remote_audio_level') {
            ...
            debug(datetime, message, data.audioLevel)
        } else {
            const eventData = {
                timeString: datetime,
                ts: timeStamp,
                message: message,
                kind: data.kind || data?.trackData?.kind || null,
                id: data.id || data?.trackData?.id || null
            }
            debug(eventData);
            ...
            if (local_events.includes(message))
                localTrackEvents.push(eventData);
            else if (remote_events.includes(message))
                remoteTrackEvents.push(eventData);
            else
                console.error(`unprocesed event: ${message}`, event);
        }
    });
